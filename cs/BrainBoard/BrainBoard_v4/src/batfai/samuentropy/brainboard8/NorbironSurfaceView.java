...
class Nodes {
	...
	private java.util.ArrayList<android.graphics.Bitmap> imgs;
	...
	{
		R.drawable.pcb550i, R.drawable.pcb550i_red,
		R.drawable.brainboard_board
	};
    ...
    public Nodes(NorbironSurfaceView surfaceView)
	{
		...
		imgs = new java.util.ArrayList<android.graphics.Bitmap>();	
        ...
        bitmap = android.graphics.BitmapFactory.decodeResource(surfaceView.getResources(), imgIDs[0]);
        bitmap = android.graphics.Bitmap.createScaledBitmap(bitmap, SLOT_SIZE, SLOT_SIZE, false);
		imgs.add(bitmap);

        bitmap = android.graphics.BitmapFactory.decodeResource(surfaceView.getResources(), imgIDs[1]);
        bitmap = android.graphics.Bitmap.createScaledBitmap(bitmap, SLOT_SIZE, SLOT_SIZE, false);
		imgs.add(bitmap);

        bitmap = android.graphics.BitmapFactory.decodeResource(surfaceView.getResources(), imgIDs[2]);
        bitmap = android.graphics.Bitmap.createScaledBitmap(bitmap, SLOT_SIZE*4, SLOT_SIZE, false);
		imgs.add(bitmap);
	...
	public android.graphics.Bitmap getImg(int i)
	{
		return imgs.get(i);
	}
...
public class NorbironSurfaceView extends android.view.SurfaceView implements Runnable {
    ...
    protected void onSizeChanged(int newx, int newy, int x, int y)
	{
        ...
        width = newx;
        height = newy;
        swidth = width / 2 - nodes.getImg(0).getWidth() / 2;
        sheight = height / 2 - nodes.getImg(0).getHeight() / 2;
    ...
    public void onDraw(android.graphics.Canvas canvas)
	{
        ...
        if (surfaceHolder.getSurface().isValid())
		{
			...
			for(int i = 0; i<6; i++)
			{
				canvas.drawBitmap(nodes.getImg(1), -startsx + 0 + i * SLOT_SIZE, -startsy + 0 + 0 * SLOT_SIZE, null);				
			}

			canvas.drawBitmap(nodes.getImg(1), -startsx + 0 + 0 * SLOT_SIZE, -startsy + 0 + 1 * SLOT_SIZE, null);				

			canvas.drawBitmap(nodes.getImg(1), -startsx + 0 + 5 * SLOT_SIZE, -startsy + 0 + 1 * SLOT_SIZE, null);				

			canvas.drawBitmap(nodes.getImg(2), -startsx + 0 + 1 * SLOT_SIZE, -startsy + 0 + 1 * SLOT_SIZE, null);				

			for(int i = 0; i<6; i++)
			{
				canvas.drawBitmap(nodes.getImg(1), -startsx + 0 + i * SLOT_SIZE, -startsy + 0 + 2 * SLOT_SIZE, null);				
			}
            ...
            for (int i = 0; i < 10; ++i)
			{
                for (int j = 0; j < 10; ++j)
				{
                   canvas.drawBitmap(nodes.getImg(0), -startsx + 0 + i * SLOT_SIZE, -startsy + 0 + j * SLOT_SIZE + 3*SLOT_SIZE, null);
                }
            }
