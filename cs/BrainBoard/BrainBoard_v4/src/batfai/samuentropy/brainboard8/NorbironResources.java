...
public class NorbironResources
{
	...
	public void initResources()
	{
		...
		{
			NorbironResource norbironResource = new NorbironResource(BLOCK_SIZE, BLOCK_SIZE,procIDs[i], surfaceView);
			neuronBoxes.add(new NeuronBox(neuronSprite, 2 * 14, 64, 62, 10, norbironResource.getBitmap(), 0, 0));
		}
		for(int i = 0; i<imgIDs.length; i+=3)
		{
			NorbironResource norbironResource = new NorbironResource(imgIDs[i+1], imgIDs[i+2], imgIDs[i], surfaceView);
			images.add(norbironResource.getBitmap());
		}
		for(int i = 0; i<menuIDs.length; i++)
		{
			NorbironResource norbironResource = new NorbironResource(BLOCK_SIZE, BLOCK_SIZE, menuIDs[i], surfaceView);
